...
class ContinuousDeduplicationScanner:
    ...
    def __init__(self):
        ...
        # Platform rotation
        self.platforms = ['ebay', 'craigslist', 'olx', 'marktplaats']
    ...
    async def run_continuous_scanner(self):
        ...
        try:
            while True:  # Run forever
                ...
                logging.info(f"   Keywords: {', '.join(keyword_batch[:3])}{'...' if len(keyword_batch) > 3 else ''}")
                ...
                hourly_unique = int(self.total_unique * 3600 / total_runtime) if total_runtime > 0 else 0
                daily_unique = hourly_unique * 24
                
                logging.info(f"📊 Cycle {cycle_count} results:")
                logging.info(f"   Raw results: {len(raw_results)}")
                logging.info(f"   Unique results: {len(unique_results)} ({dedup_rate:.1f}% new)")
                ...
                logging.info(f"   Deduplication saved: {self.total_scanned - self.total_unique:,} duplicates")
    ...
    async def scan_ebay_batch(self, keywords: List[str]) -> List[Dict]:
        ...
        try:
            ...
            cutoff_date = (datetime.now() - timedelta(days=1)).strftime("%Y-%m-%dT%H:%M:%S.000Z")
    ...
    def count_wildlife_hits(self, results: List[Dict]) -> int:
        ...
        for result in results:
            ...
            high_indicators = ['ivory', 'rhino', 'tiger', 'elephant', 'pangolin', 'bear bile', 'leopard']
    ...
    async def generate_progress_report(self, cycle_count: int, runtime: float):
        ...
        logging.info(f"   • Wildlife hits: {self.wildlife_hits:,}")
        logging.info(f"   • Deduplication rate: {((self.total_scanned - self.total_unique) / self.total_scanned * 100):.1f}%")
        logging.info(f"   • Hourly unique rate: {int(self.total_unique / hours_running):,}")
        logging.info(f"   • Daily projection: {int(self.total_unique * 24 / hours_running):,}")
    ...
    async def generate_final_report(self, cycle_count: int, runtime: float):
        ...
        logging.info(f"   • Wildlife hits: {self.wildlife_hits:,}")
        logging.info(f"   • Duplicates prevented: {self.total_scanned - self.total_unique:,}")
        logging.info(f"   • Average hourly unique: {int(self.total_unique / hours_running):,}")
        logging.info(f"   • Honest daily projection: {int(self.total_unique * 24 / hours_running):,}")
